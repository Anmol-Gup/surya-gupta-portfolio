...
const Navbar: React.FC<NavbarProp> = ({navColor}) => {
    ...
    return (<>
        ...
        <nav className={`${navColor ? 'bg-[#2b2d33]' : 'bg-black'} lg:static basis-1/2 absolute top-full left-0 lg:top-0 w-full bg-[#2b2d33]`}>
            {toggle && <ul className={`transition-all duration-500 ease-in-out text-left font-medium lg:w-full lg:h-full flex lg:flex-row lg:justify-evenly flex-col lg:gap-4 gap-2 text-white font-medium px-3 py-4`}>
                ...
                <li className='my-3 lg:my-0'><a href='#' className='list-none decoration-none border-2 border-orange rounded-full px-3 py-2 transition-all ease cv' download>Download CV</a></li>
            </ul>}
        </nav>
    </>)
}
